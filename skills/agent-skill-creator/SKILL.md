...
6. Create `skills/<skill-name>/`, where `<skill-name>` must use lowercase kebab-case. This creator contract uses `skills/` as the canonical authoring target; any `.<platform>/skills/` path is a compatibility or projection surface, not the source of truth.
...
- Do not treat any `.<platform>/skills/` compatibility/projection, promotion, or runtime/tooling follow-up as part of this drafting step.
