...
# Process
1. Detect release context first: normal PR gate, normal tag, or emergency path.
...
4. Hard-block when the target tag already exists, when the workspace is dirty, when version sources conflict, or when the PR gate is not met.
5. Treat the PR gate as strict. Require reviewer approval, CI green, base tests passing, strict type checks passing, lint passing, documentation updated where contracts changed, versions synchronized, and a clean workspace.
...
8. Derive the recommended bump direction from accumulated commit semantics: breaking changes outrank features, and features outrank fixes or maintenance.
9. Allow emergency mode only for reviewer-timing bypass. Do not bypass tests, typing, lint, version sync, clean workspace, or tag-conflict checks.
10. Require explicit emergency markers, human confirmation, and release-note disclosure before using the emergency path.
...
- Do not auto-bypass core gates such as CI, type checks, lint, version sync, clean workspace, or tag uniqueness.
