...
- [ ] Main Agent performs the Phase 4.5 planner contract-alignment check
  against the approved topic contract and reviewed seven-file package.
- [ ] If alignment passes, stop at STOP POINT 1 for explicit human
  authorization before any commit, push, or PR action.
- [ ] If alignment detects scope, contract, ownership, path, or release-timing
  drift, return only the affected work to the declared owner; do not publish.

### feature PR and human merge handoff

- [ ] Complete pre-commit validation and receive STOP POINT 1 human
  authorization before commit / push / PR.
- [ ] Open the one feature PR with only topic artifacts and the skill package;
  do not add catalog or version changes.
- [ ] At STOP POINT 2, stop completely for human merge handoff. Do not poll.
...
- Creator never writes the review log, summary, `README.md`, or `VERSION`.
  Reviewer is independent from Creator. The topic is `approved` pending Main
  Agent's Phase 4.5 alignment; Main Agent owns the deferred Phase 10 metadata
  and tag route after explicit resume.
