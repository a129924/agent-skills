...
- `artifacts/skills-inventory.jsonl` is regenerated by the existing canonical
  inventory builder and contains the final single record for
  `canonical_path` `skills/scope-draft-plan` (no trailing slash)
...
- **In scope**:
  - Create and maintain these topic-local planning / routing artifacts:
    ...
    - `plan/scope-draft-plan/scope-draft-plan.correction-plan.md`
    - `plan/scope-draft-plan/scope-draft-plan.correction-step.md`
  ...
  - Regenerate the generated canonical inventory snapshot at
    `artifacts/skills-inventory.jsonl` with the existing
    `scripts/build_skills_inventory.py` builder. This corrects the PR #119 P1
    inventory drift; it is not a new inventory feature or a new Mission.
...
- **Out of scope**:
  ...
  - Changes to the inventory builder, its tests, or the inventory record
    contract; this topic only regenerates its existing canonical snapshot
...
- This is one stable-library topic with deferred release timing. PR #119 stays
  on the managed feature worktree
  `../agent-skills.worktrees/agent-20260724-scope-draft-plan` and branch
  `feat/andrew/scope-draft-plan`. Its correction loop may write only the exact
  paths in `Artifact Paths`: the parent topic artifacts, the two correction
  artifacts, the seven canonical skill files, and the generated inventory
  snapshot. `README.md` and `VERSION` remain forbidden from the feature PR.
...
- Planning actor owns the parent topic artifacts and correction artifacts;
  Implementer owns the bounded correction writes to the existing skill files
  and generated inventory snapshot; Reviewer owns independent verdicts; Main
  Agent owns branch, worktree, PR, human-gate, post-merge, and deferred-release
  routing.
- Creator implementation must not update the topic plan, review log, summary,
  ...
  Main Agent owners.
- The open feature PR's correction write set is exactly the paths in `Artifact
  Paths`: parent and correction planning artifacts, the existing seven-file
  canonical skill package, and `artifacts/skills-inventory.jsonl`. The original
  publication's 11 paths are historical evidence, not the current allowed
  write-set limit. `README.md` and `VERSION` are written only by Main Agent's
  Phase 10 action after merge and explicit resume; they must not cause a
  transition from `merged` back to `publish-in-progress`.
...
- PR #119 has a planner-confirmed `medium` inventory-drift correction in
  `PLANNER_REPLAN`. The correction stays within this open PR and ordinary PR
  comment loop: it creates no new worktree, PR, release route, or Human Gate.
...
| Correction plan | `plan/scope-draft-plan/scope-draft-plan.correction-plan.md` | Planner | Historical correction contract for PR #119 medium inventory drift; does not replace parent truth |
| Correction progression | `plan/scope-draft-plan/scope-draft-plan.correction-step.md` | Planner / Implementer | Historical correction sequence and bounded implementer handoff |
...
| Inventory builder | `scripts/build_skills_inventory.py` | Implementer | Read-only existing builder used to regenerate the canonical snapshot; its behavior is not changed |
| Generated canonical inventory snapshot | `artifacts/skills-inventory.jsonl` | Implementer | Correction write path; regenerated final snapshot with exactly 57 canonical-skill records, including exactly one record with `canonical_path` `skills/scope-draft-plan` (no trailing slash) |
...
- The PR #119 correction write set is exactly the four parent topic artifacts,
  two correction artifacts, seven canonical skill files, and generated
  `artifacts/skills-inventory.jsonl` snapshot listed above. `README.md` and
  `VERSION` are excluded from that PR and belong only to Main Agent's deferred
  Phase 10 action.
...
6. Stop at `review-ready` and hand the canonical skill package plus this topic
   ...
   `VERSION` in feature work. The PR #119 correction sequence is owned by the
   declared correction artifacts and does not redefine these creator steps.
...
- Correction check: the inventory builder regenerates the snapshot to exactly
  57 records; exactly one record has `canonical_path` equal to
  `skills/scope-draft-plan` (no trailing slash); and the correction diff does
  not alter any pre-existing inventory record. The same loop independently
  verifies exactly four bounded PR-comment repairs: the output template permits
  only a BC Mission (or explicitly blocks Cross-BC / Spike), the step and
  summary remove or correct stale `uncommitted by design` wording, and the
  review log normalizes the skill-review gate verdict to `approved` (`PASS` is
  explanatory prose only). These repairs do not add scope.
