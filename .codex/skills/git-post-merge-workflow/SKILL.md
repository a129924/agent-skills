...
description: "Inspect and perform authorized cleanup and fast-forward sync after a PR has merged."
complexity: medium
risk_profile:
...
5. Detect the repository's remote and default branch dynamically. Sync the default branch with `git pull --ff-only <remote> <default-branch>` to ensure FF-only semantics; if upstream is configured, `git pull --ff-only` is acceptable; otherwise use explicit remote+branch form.
