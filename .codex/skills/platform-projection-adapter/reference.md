...
- The projected script must remain runnable from `.codex/skills/...` without
...
- Placeholder text that starts with `.codex/` is rewritten in target
...
- When the skill text describes a projection surface abstractly, prefer
  `.codex/...` wording over naming one concrete platform path.
