...
- output-facing / runnable / copy-pasteable: `.codex/skills/<skill-name>/`
...
- author new or materially transitioned canonical source content under
  `skills/<skill-name>/`
- use `.codex/skills/<skill-name>/` as the default output-facing,
...
- use `skills/<skill-name>/` as a projected path only when the projected
  ...
  bootstrap fallback
- do not assume any concrete `.codex/skills/` path unless context or
...
- if downstream planning-spine skills still assume a `.codex/skills/`
