...
6. Create `skills/<skill-name>/`, where `<skill-name>` must use lowercase kebab-case. This creator contract uses `skills/` as the canonical authoring target; any `.codex/skills/` path is a compatibility or projection surface, not the source of truth, and should be mentioned only when context or prompt explicitly injects that surface into scope.
...
- Do not assume any concrete `.codex/skills/` surface unless context or
  prompt explicitly puts that compatibility/projection path in scope.
- Do not treat any `.codex/skills/` compatibility/projection, promotion,
